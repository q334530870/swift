...
// label setting
let HRToastMaxWidth       :  CGFloat  = 0.2;      // 80% of parent view width 百分比
let HRToastMaxHeight      :  CGFloat  = 0.2;
    ...
    */
    ...
    func showToast(toast toast: UIView, duration: Double, position: AnyObject) {
        ...
        UIView.animateWithDuration(HRToastFadeDuration,
            delay: 0.0, options: [UIViewAnimationOptions.CurveEaseOut,UIViewAnimationOptions.AllowUserInteraction],
            animations: {
                toast.alpha = 1.0
            },
    ...
    func makeToastActivity(position pos: AnyObject, message msg: String = "",controller:UIViewController? = nil) {
		let existToast = objc_getAssociatedObject(self, &HRToastView) as! UIView?
		        if existToast != nil {
		            if let timer = objc_getAssociatedObject(existToast, &HRToastTimer) as? NSTimer {
		                timer.invalidate();
