...
class SecondHandTableViewController: UITableViewController,UISearchBarDelegate,UISearchResultsUpdating {
    ...
    override func viewDidLoad() {
        ...
        getData()
        //下拉刷新
        let header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(SecondHandTableViewController.refresh))
        header.lastUpdatedTimeLabel?.hidden = true
        self.tableView.mj_header = header
        //上拉加载
        self.tableView.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(SecondHandTableViewController.loadData))
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0{
            ...
            button.addTarget(self, action: #selector(SecondHandTableViewController.buy(_:)), forControlEvents: UIControlEvents.TouchUpInside)
