...
class GatherBuyTableViewController: UITableViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        getProduct()
        titleList = ["请选择出售产品","1-是否接受初次发行？","2-是否接受T+1交易（全款24小时内付清）？","3-是否接受集合竞价交易？","4-是否是发行人的回购？","5-认购下单，可接受支付的选项（可多选）","(T+1) 保证金","(T+1) 首付款","(T+1) 全款","(T+0) 全款","购买数量/片","过息利息金额","最低收益率（%）","竞价开始日期","竞价结束日期"]
